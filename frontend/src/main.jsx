...
function App() {
  ...
  async function runWithPayload(payload) {
    ...
    try {
      ...
      setResult(res);
      setLastQuery(payload.natural_language_query || "");
  ...
  // Prefer dimension that is best for chart axis (quarters/years show as 2024-Q1, 2022; region/country as names)
  ...
  // Use actual measure in table: profit margin query has profit_current/profit_growth, not revenue
  ...
  const isCompound = result?.compound === true;
  const step0Result = result?.step0?.result;
    ...
    >
      ...
      >
        ...
        >
          ...
          {result && lastQuery && followUpSuggestions.length > 0 && (
          ...
          {/* Assistant response bubble with agents + visuals */}
          {result && lastQuery && (
            <div style={{ display: "flex", justifyContent: "flex-start" }}>
              <div
                style={{
              ...
              >
                <div style={{ marginBottom: "0.75rem", display: "flex", alignItems: "center", gap: "0.4rem" }}>
                  ...
                  <div style={{ fontSize: "0.85rem", color: "#0f172a" }}>
                    {isCompound ? `Part 1 & 2: ${result?.step0?.query ?? "Part 1"}, then ${result?.step1?.query ?? "Part 2"}.` : "Multi-agent analysis: cube, KPIs, drill-down and report."}
                  </div>
                </div>
                ...
                {isCompound && step0CubeData.length > 0 && (
                  ...
                  >
                    <h3 style={{ margin: "0 0 0.5rem 0", fontSize: "0.95rem" }}>Part 1: {result?.step0?.query ?? "Revenue by year"}</h3>
                    <p style={{ margin: "0 0 0.5rem 0", fontSize: "0.8rem", color: "#1e40af" }}>
                      {step0CubeCols.includes("date_year") ? "Revenue breakdown by year (2022, 2023, 2024)." : "Totals by category."}
                    </p>
                    {renderTable(step0CubeData, step0CubeCols, false)}
                  </section>
                )}

                {isCompound && step0CubeData.length > 0 && (
                  <h3 style={{ margin: "0 0 0.5rem 0", fontSize: "0.95rem", color: "#0f172a" }}>Part 2: {result?.step1?.query ?? "Drill into year by quarter"}</h3>
                )}

                <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                  {/* 1. Cube Operations Agent - always show card */}
                  <section
                    style={{
                      padding: "0.75rem 0.85rem",
                      background: "#fff7ed",
                  ...
                  >
                    ...
                    {cubeResult ? (
                      <>
                        <p style={{ margin: "0 0 0.4rem 0", fontSize: "0.8rem", fontWeight: 600 }}>
                          Operation: {cubeResult.operation || cubeResult.type || "dice"}
                        </p>
                        {renderTable(cubeData, cubeCols, false)}
                      </>
                    ) : (
                      <p style={{ margin: 0, color: "#92400e", fontStyle: "italic", fontSize: "0.8rem" }}>
                        Not used for this query.
                      </p>
                    )}
                  </section>

                  {/* 2. KPI Calculator Agent - always show card */}
                  <section
                    style={{
                      padding: "0.75rem 0.85rem",
                      background: "#f5f3ff",
                  ...
                  >
                    ...
                    {(kpiResult || result?.kpi_data) ? (
                      <>
                        <p style={{ margin: "0 0 0.35rem 0", fontSize: "0.8rem", fontWeight: 600 }}>
                          Applied: {kpiResult?.kpi_type || "yoy_growth"}
                        </p>
                        {bestPerformer && (
                          <p style={{ margin: "0 0 0.4rem 0", fontSize: "0.8rem" }}>
                            Best performer:{" "}
                            <strong>
                              {bestPerformer.region ?? bestPerformer.date_year ?? Object.values(bestPerformer)[0]}
                            </strong>
                          </p>
                        )}
                      ...
                      <p style={{ margin: 0, color: "#5b21b6", fontStyle: "italic", fontSize: "0.8rem" }}>
                        Not used for this query.
                      </p>
                    )}
                  </section>
                  ...
                  {/* 3. Dimension Navigator Agent - always show card */}
                  <section
                    style={{
                      padding: "0.75rem 0.85rem",
                      background: "#eff6ff",
                  ...
                  >
                    ...
                    {drill ? (
                      <>
                        <p style={{ margin: "0 0 0.35rem 0", fontSize: "0.8rem" }}>
                          Current level: <strong>{drill.current_level}</strong> → Next level:{" "}
                          <strong>{drill.next_level}</strong>
                        </p>
                        {drill.members?.length > 0 && (
                          <p style={{ margin: 0, fontSize: "0.8rem" }}>
                            Members: {drill.members.map((m) => m.value).join(", ")}
                          </p>
                        )}
                      </>
                    ) : (
                      <p style={{ margin: 0, color: "#1e40af", fontStyle: "italic", fontSize: "0.8rem" }}>
                        Not used for this query.
                      </p>
                    )}
                  </section>

                  {/* 4. Report Generator Agent - always show card */}
                  <section
                    style={{
                      padding: "0.75rem 0.85rem",
                      background: "#ecfdf5",
                    ...
                    {report ? (
                      <>
                        ...
                        {formattingRules.length > 0 && (
                          <p style={{ margin: "0 0 0.5rem 0", fontSize: "0.8rem", color: "#047857" }}>
                        ...
                        {table.length === 0 ? (
                          <p style={{ margin: 0, fontSize: "0.8rem" }}>No rows.</p>
                        ) : (
                          renderTable(table, columns, true)
                        )}
                      </>
                    ) : (
                      <p style={{ margin: 0, color: "#065f46", fontStyle: "italic", fontSize: "0.8rem" }}>
                        Not used for this query.
                      </p>
                    )}
                  </section>
